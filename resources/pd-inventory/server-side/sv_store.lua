			...
			if cfg.storeTypes[type].permission then
				if vRP.hasPermission(user_id, cfg.storeTypes[type].permission) then
					...
					if vRP.tryPayment(user_id, price * number, true) then
						...
						if item == "pistol" or item == "ammo_pistol" then
							if item == "pistol" then item = "M1911" end
							if item == "ammo_pistol" then item = "Munição M1911" end
							SendWebhookMessage(webhookporte,"```prolog\n[OFICIAL]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[PEGOU]: "..number.." "..item.." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
						end
					else

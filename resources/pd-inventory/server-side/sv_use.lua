...
AddEventHandler("b03461cc:pd-inventory:useItem", function(item, amount)
	...
	if user_id then
		if getItemAmount(user_id, item) >= amount then
							...
							if pp >= 3 then
								...
								if nplayer then
									if vRPclient.getHealth(nplayer) <= 101 then
										...
										SendWebhookMessage(webhookre,"```prolog\n[POLICIAL]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[=============REANIMOU=============] \n[PASSAPORTE]: "..nuser_id.." "..uidentity.name.." "..uidentity.firstname.." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
									else
										TriggerClientEvent("Notify",source,"importante","A pessoa precisa estar em coma para prosseguir.",8000)
									end
								end
			...
			if item == "radio" then
				TriggerClientEvent("DLGames:openRadio",source)
			end

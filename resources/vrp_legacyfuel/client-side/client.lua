...
AddEventHandler('fuel:startFuelUpTick',function(pumpObject,ped,vehicle)
	...
	while isFueling do
		...
		local fuelToAdd = math.random(2,8) / 100.0
...
AddEventHandler('fuel:refuelFromPump',function(pumpObject,ped,vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"PRESSIONE ~g~E ~w~PARA CANCELAR",370)
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 0.5,"TANQUE: ~y~"..Round(currentFuel,1).."%",370)
		else
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 0.5,"PRESSIONE ~g~E ~w~PARA CANCELAR",370)
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 0.34,"GALÃO: ~b~"..Round(GetAmmoInPedWeapon(ped,883325847) / 4500 * 100,1).."%~w~    TANQUE: ~y~"..Round(currentFuel,1).."%",370)
		end
		...
		if IsControlJustReleased(0,38) and IsInputDisabled(0) or DoesEntityExist(GetPedInVehicleSeat(vehicle,-1)) or (isNearPump and GetEntityHealth(pumpObject) <= 0) then
			if cooldown < 1 then
				cooldown = 5
				isFueling = false
			end
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped),-1) == ped then
				...
				DrawText3Ds(pumpCoords.x,pumpCoords.y,pumpCoords.z + 1.2,"SAIA DO ~y~VEÍCULO ~w~PARA ABASTECER",370)
			else
				local vehicle = GetPlayersLastVehicle()
				local vehicleCoords = GetEntityCoords(vehicle)
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped),vehicleCoords) < 2.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle,-1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 99 and canFuel then
							DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"PRESSIONE ~g~E ~w~PARA ABASTECER",370)
						...
						elseif not canFuel then
							DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"~o~GALÃO VAZIO",370)
						else
							DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"~g~TANQUE CHEIO",370)
						end
					end
				elseif isNearPump then
					local stringCoords = GetEntityCoords(isNearPump)
					DrawText3Ds(stringCoords.x,stringCoords.y,stringCoords.z + 1.2,"PRESSIONE ~g~E ~w~PARA COMPRAR UM ~b~GALÃO DE GASOLINA",370)
					if IsControlJustReleased(0,38) and IsInputDisabled(0) then

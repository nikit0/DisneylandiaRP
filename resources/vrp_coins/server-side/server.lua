...
RegisterCommand('viptime',function(source,args,rawCommand)
    ...
    local rows = vRP.query("getVipTime",{ id = user_id })
...
RegisterCommand('rentaltime',function(source,args,rawCommand)
    local user_id = vRP.getUserId(source)
    local rows = vRP.query("getRentalTime",{ user_id = user_id })
    if #rows then 
...
AddEventHandler("vRP:playerSpawn",function(user_id,source,first_spawn)
    ...
    if #rows then
        ...
        if rows[1].vip_time ~= 0 and os.time() >= rows[1].vip_time then
            ...
            vRP.execute("updateVipTime",{ id = user_id, vip_time = 0 })
        end
    end
        -- Checa o tempo do Veículo
    local rows2 = vRP.query("getRentalTime",{ user_id = user_id })
    if #rows2 then 
        for k,v in pairs(rows2) do
            if vRP.vehicleClass(tostring(v.vehicle)) == "rental" then
                if v.rental_time ~= 0 and os.time() >= v.rental_time then
                    vRP.execute("creative/rem_vehicle",{ user_id = parseInt(user_id), vehicle = v.vehicle })
                end
            end
        end
    end
...
function src.giveVehicle(vehicle)
    ...
    if vehicle then
        if src.remCoins(coinsShop[vehicle.name].price) then
            vRP.execute("creative/add_vehicle",{ user_id = parseInt(user_id), vehicle = vehicle.name, ipva = parseInt(os.time()) })
            vRP.execute("setRentalTime",{ user_id = parseInt(user_id), vehicle = vehicle.name, rental_time = parseInt(os.time()+30*24*60*60) })
            ucoins = vRP.getCoins(user_id)
        end
    end
    return ucoins

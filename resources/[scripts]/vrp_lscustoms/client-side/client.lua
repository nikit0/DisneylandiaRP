...
local garages = {
	...
	[8] = { locked = false, camera = { x = 1770.31, y = 3333.68, z = 41.44, h = 147.67 }, driveout = { x = 1767.7, y = 3329.46, z = 40.82, heading = 297.30 }, drivein = { x = 1767.7, y = 3329.46, z = 40.82, heading = 297.30 }, outside = { x = 1767.7, y = 3329.46, z = 40.82, heading = 297.30 }, inside = { x = 1767.7, y = 3329.46, z = 40.82, heading = 297.30 } }
}
	...
	if DoesEntityExist(veh) then
		...
		if IsThisModelABike(GetEntityModel(veh)) then
			...
			for n, w in pairs(LSC_Config.prices.importwheels) do
				local btn = importw:addPurchase(w.name, w.price)
				btn.wtype = w.wtype
				btn.modtype = 23
				btn.mod = w.mod
			end
								...
								if not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
											if IsControlJustPressed(1, 38) and IsInputDisabled(0) then
												if emP.checkPermission() then
													...
													TriggerEvent("Notify", "negado", "Somente <b>Mecânicos</b> podem modificar os veículos.", 8000)
												end
											else
												DrawText3Ds(outside.x, outside.y, outside.z, "~r~E ~w~ PARA ACESSAR A ~y~ BENNYS", 370)
											end
										else
											DrawText3Ds(outside.x, outside.y, outside.z, "~y~BENNYS ~w~ ESTÁ OCUPADA NO MOMENTO", 370)
										end
									else
										if IsControlJustPressed(1, 38) and IsInputDisabled(0) then
											if emP.checkPermission() then
												...
												currentgarage = i
												DriveInGarage()
												TriggerEvent("checkcam", source, false)
											else
												TriggerEvent("Notify", "negado", "Somente <b>Mecânicos</b> podem modificar os veículos.", 8000)
											end
										else
								...
								if not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
											...
											DrawText3Ds(x, y, z, "~y~BENNYS ~w~ ESTÁ OCUPADA NO MOMENTO", 370)
										end
									else
										inside = true
										currentpos = pos
...
function LSCMenu:OnMenuClose(m)
	...
	TriggerEvent("checkcam", source, true)
end
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	if m == "horn" then
		...
		if IsHornActive(veh) or IsControlPressed(1, 86) and IsInputDisabled(0) then
...
CreateThread(function()
	while true do
		...
		if LSCMenu:isVisible() then
			DrawIbuttons()
			if IsDisabledControlJustPressed(1, 0) or IsControlJustPressed(1, 0) and IsInputDisabled(0) then

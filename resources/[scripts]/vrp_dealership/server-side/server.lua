	...
	if user_id then
		...
		if vehicle[1] then
			...
			if vRP.tryFullPayment(user_id,vRP.vehiclePrice(name)*tax) then
				vRP.execute("set_stock",{ spawn = name, stock = parseInt(rows2.stock) - 1 })
				vRP.execute("creative/add_vehicle",{ user_id = parseInt(user_id), vehicle = name, ipva = parseInt(os.time()) })

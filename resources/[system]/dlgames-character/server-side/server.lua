...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("dlgames-character:characterCreate",source)
	end
end

RegisterServerEvent("dlgames-character:finishedCharacter")
AddEventHandler("dlgames-character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("dlgames-character:normalSpawn",source,firstspawn)
	TriggerEvent("dlgames-barbershop:init",user_id)

	...
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if newstate ~= state_airmanu[veh] then
			...
			elseif newstate == 3 then
				...
				TriggerEvent("vehcontrol:ELSClick","update",0.7)
			end
...
---------------------------------------------------------------------
AddEventHandler('vehcontrol:ELSClick',function(soundFile,soundVolume)
					...
					if not IsPauseMenuActive() then
						if IsDisabledControlJustReleased(0,85) then
							if IsVehicleSirenOn(veh) then
								TriggerEvent("vehcontrol:ELSClick","offline",0.7)
								SetVehicleSiren(veh,false)
							else
								TriggerEvent("vehcontrol:ELSClick","online",0.5)
								Citizen.Wait(150)
						...
						elseif IsDisabledControlJustReleased(0,19) then
							...
							if cstate == 0 then
								...
								TriggerEvent("vehcontrol:ELSClick","down",1)
								SetLxSirenStateForVeh(veh,0)
								count_bcast_timer = delay_bcast_timer
							end
						end

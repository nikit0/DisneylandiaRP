    ...
    if user_id then
        if vRP.hasPermission(user_id,"admin.permissao") then
            ...
            for k,v in pairs(users) do
                ...
                if identity then
                    TriggerClientEvent("gcPhone:receiveMessage",id,_internalAddMessage("4002-8922",identity.phone,"Governo informa: "..messages,0))
                end
            end

            TriggerClientEvent("Notify",-1,"negado","O governo enviou uma mensagem no seu telefone.",5000)
        end
    end
end)
...
function gcp.checkCelular()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if exports["pd-inventory"]:getItemAmount(user_id,"celular") >= 1 then
			return true
		else
			TriggerClientEvent("Notify",source,"importante","Você precisa de um <b>Celular</b>, dirija-se até uma <b>Loja de Departamento</b><br>para realizar a compra.",8000)
			return false
		end
	end
end
